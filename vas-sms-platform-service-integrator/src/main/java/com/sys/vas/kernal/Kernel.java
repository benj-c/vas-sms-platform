...
public class Kernel {
    ...
    public JSONObject getFlowChart(String api, String xml) {
        String code1 = "";
        String code2 = "";
        try {
            code1 = "st=>start: START|past\nEND=>end: END";
            code2 = "st->B0";
            ...
            for (int b = 0; b < blocklist.getLength(); b++) {
                currentBlock = (Element) blocklist.item(b);
                currentBlockType = currentBlock.getAttribute("type");
                if (currentBlockType.equals("func")) {
                    code1 = code1 + "\nB" + currentBlock.getAttribute("id") + "=>operation: ";
                    ...
                    String prastr = "";
                    code1 = code1 + "System ," + className + "\nCall ," + functionName + "\n";
                    for (int j = 0; j < nListparam.getLength(); j++) {
                        Node pNode = nListparam.item(j);
                        if (pNode.getNodeType() == Node.ELEMENT_NODE) {
                            Element ele = (Element) pNode;
                            String var = ele.getTextContent().replaceAll("\\|\\|", "or").replaceAll("\\(", "[")
                                    .replaceAll("\\)", "]").replaceAll(":", ";").replaceAll(":", ";")
                                    .replaceAll("&&", "and").replaceAll("&", ":");
                            log.debug("parameterName:variable:value |" + ele.getAttribute("name") + ":"
                                    + ele.getTextContent() + ":[" + var + "]");
                            prastr = prastr + ele.getAttribute("name") + ":" + var + ",";
                            code1 = code1 + "ip-" + ele.getAttribute("name") + " , " + breakLine(var, 50) + "\n";
                        }
                    }
                    code1 = code1 + "|past";
                    code2 = code2 + "\nB" + currentBlock.getAttribute("id") + "->B" + nextBlock;
                } else if (currentBlockType.equals("assign")) {
                    code1 = code1 + "\nB" + currentBlock.getAttribute("id") + "=>operation: Set atributes\n";
                    String nextBlock = currentBlock.getElementsByTagName("next-node").item(0).getTextContent();
                    NodeList nListparam = currentBlock.getElementsByTagName("variable");
                    for (int j = 0; j < nListparam.getLength(); j++) {
                        Node pNode = nListparam.item(j);
                        if (pNode.getNodeType() == Node.ELEMENT_NODE) {
                            Element ele = (Element) pNode;
                            String vName = ele.getAttribute("name");
                            String fc = ele.getAttribute("fc");
                            String vVal = ele.getTextContent().replaceAll("\\\\", "").replaceAll("\\|\\|", "or")
                                    .replaceAll("\\(", "[").replaceAll("\\)", "]").replaceAll(":", ";")
                                    .replaceAll(":", ";").replaceAll("&&", "and").replaceAll("\\|", "pipe");
                            if (fc != null) {
                                if (fc.equalsIgnoreCase("ignore"))
                                    vVal = "function...";
                            }
                            log.info(
                                    "Property name:variable:value|" + vName + ":" + ele.getTextContent() + ":" + vVal);
                            code1 = code1 + "var-" + vName + " , " + breakLine(vVal, 50) + "\n";
                        }
                    }
                    code1 = code1 + "|current";
                    code2 = code2 + "\nB" + currentBlock.getAttribute("id") + "->B" + nextBlock;
                } else if (currentBlockType.equals("pass")) {
                    code1 = code1 + "\nB" + currentBlock.getAttribute("id") + "=>operation: \n";
                    String nextBlock = currentBlock.getElementsByTagName("next-node").item(0).getTextContent();
                    code2 = code2 + "\nB" + currentBlock.getAttribute("id") + "->B" + nextBlock;
                } else if (currentBlockType.equals("branch")) {
                    ...
                    for (int j = 0; j < nListparam.getLength(); j++) {
                        ...
                        if (pNode.getNodeType() == Node.ELEMENT_NODE) {
                            Element ele = (Element) pNode;
                            log.info("Case Id:|" + ele.getAttribute("id"));
                            nextBlock = ele.getElementsByTagName("next-node").item(0).getTextContent();
                            //////////////////////////////////////////////////////////////////////////////////////////////
                            String expression = ele.getElementsByTagName("expression").item(0).getTextContent();
                            expression = expression.replaceAll("\\|\\|", "or").replaceAll("\\(", "[")
                                    .replaceAll("\\)", "]").replaceAll(":", ";").replaceAll(":", ";")
                                    .replaceAll("&&", "and");
                            expression = expression.replaceAll("\\\\", "");
                            String curBId = "";
                            if (j == 0)
                                curBId = currentBlock.getAttribute("id");
                            else
                                curBId = currentBlock.getAttribute("id") + "_" + ele.getAttribute("id");
                            code1 = code1 + "\nB" + curBId + "=>condition: " + breakLine(expression, 20) + "\n";// chnged
                            code1 = code1 + "|approved";
                            NodeList nListvar = ele.getElementsByTagName("variable");
                            boolean haveVar = false;
                            if (nListvar != null) {
                                if (nListvar.getLength() > 0) {
                                    haveVar = true;
                                    code1 = code1 + "\nB" + curBId + "_V=>operation: Set atributes\n";// chnged
                                }
                                for (int k = 0; k < nListvar.getLength(); k++) {
                                    ...
                                    if (pNodeVar.getNodeType() == Node.ELEMENT_NODE) {
                                        ...
                                        String fc = elevar.getAttribute("fc");
                                        String vVal = elevar.getTextContent().replaceAll("\\\\", "")
                            ...
                            if (j < nListparam.getLength() - 1)
                                code2 = code2 + "\nB" + curBId + "(no," + side + ")->B"
                                        + currentBlock.getAttribute("id") + "_" + (j + 2);
                            else
                                code2 = code2 + "\nB" + curBId + "(no," + side + ")->B"
                                        + currentBlock.getAttribute("id") + "_default";

                            if (haveVar) {
                                code1 = code1 + "|current";
                                code2 = code2 + "\nB" + curBId + "(yes,left)->B" + curBId + "_V";
                                code2 = code2 + "\nB" + curBId + "_V->B" + nextBlock;
                            } else
                                code2 = code2 + "\nB" + curBId + "(yes,left)->B" + nextBlock;

                        }
                    }

                    Element ele = (Element) currentBlock.getElementsByTagName("default").item(0);
                    code1 = code1 + "\nB" + currentBlock.getAttribute("id") + "_default=>operation: ";
                    NodeList nListvar = ele.getElementsByTagName("variable");
                    if (nListvar != null) {
                        for (int k = 0; k < nListvar.getLength(); k++) {
                            Node pNodeVar = nListvar.item(k);
                            if (pNodeVar.getNodeType() == Node.ELEMENT_NODE) {
                                ...
                                String vVal = elevar.getTextContent().replaceAll("\\\\", "").replaceAll("\\|\\|", "or")
                                        .replaceAll("\\(", "[").replaceAll("\\)", "]").replaceAll(":", ";")
                    ...
                    nextBlock = ele.getElementsByTagName("next-node").item(0).getTextContent();
                    code2 = code2 + "\nB" + currentBlock.getAttribute("id") + "_default->B" + nextBlock;
                }

            }
            currentBlock = getBlockbyId(doc, "return", "");

            NodeList nListparam = currentBlock.getElementsByTagName("outputparams");
            code1 = code1 + "\nBEND=>operation: Return block";
            for (int j = 0; j < nListparam.getLength(); j++) {
                Node pNode = nListparam.item(j);
                if (pNode.getNodeType() == Node.ELEMENT_NODE) {
                    Element ele = (Element) pNode;
                    ...
                    if (vName.equals("last")) {
                        code1 = code1 + "\nReturning Last executed func Output";
                        break;
                    } else if (vName.equals("ses")) {
                        code1 = code1 + "\nReturning All attributes..";
                        break;
                    } else {
                        code1 = code1 + "\nvar-" + vName + " , " + breakLine(vVal, 50);
                    }
            ...
            code2 = code2 + "\nBEND->END";
            log.info("End of executing flowchart....");
