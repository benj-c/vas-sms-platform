...
public class AppConfig {
    ...
    @Value("${vas.smsr.srv-mapper.thread-pool.core-size}")
    private int corePoolSize;
    @Value("${vas.smsr.srv-mapper.thread-pool.max-size}")
    private int maxPoolSize;
    @Value("${vas.smsr.srv-mapper.thread-pool.queue-capacity}")
    private int queueCapacity;

    @Bean("serviceMapperPool")
    public Executor getNotifyExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(corePoolSize);
        executor.setMaxPoolSize(maxPoolSize);
        executor.setQueueCapacity(queueCapacity);
        executor.setThreadNamePrefix("smsr-srv-mapper-");
        executor.initialize();
        return executor;
    }
